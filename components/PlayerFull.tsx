...
const PlayerFull: React.FC<PlayerFullProps> = ({ 
  ...
  const handleDrop = (e: React.DragEvent, index: number) => {
    ...
    if (draggedItemIndex !== null && draggedItemIndex !== index) {
      onMoveQueueItem(draggedItemIndex, index);
    }
    ...
    <div className="fixed inset-0 z-[200] flex flex-col bg-black overflow-hidden animate-in slide-in-from-bottom duration-700 cubic-bezier(0.23, 1, 0.32, 1)">
      ...
      <div className="relative z-10 flex flex-col h-full px-6 pt-12 pb-6">
                ...
                {!showSleepTimerMenu ? (
                  ...
                  <div className="animate-in slide-in-from-right-4 duration-500 space-y-1">
                    ...
                    {sleepOptions.map(opt => (
                      <button 
                        key={opt.label} 
                        onClick={() => { setSleepTimer(opt.value); setShowDropdown(false); }} 
                        className={`w-full text-left p-3 rounded-2xl hover:bg-white/10 text-[10px] font-black uppercase tracking-widest transition-all ${sleepTimer === opt.value ? 'text-accent' : 'text-white/70'}`}
                      >
                        {opt.label}
                      </button>
                    ))}
        ...
        <div className="flex-1 overflow-hidden relative flex flex-col items-center">
          {activeTab === 'player' && (
            <div className="flex-1 flex flex-col justify-center items-center w-full animate-in fade-in zoom-in-95 duration-700">
              <div 
                className="w-full max-w-[240px] bg-white/[0.03] backdrop-blur-2xl rounded-[48px] p-4 border border-white/10 shadow-[0_30px_80px_rgba(0,0,0,0.8)] relative select-none"
        ...
        <footer className="mt-8 space-y-5 bg-zinc-900/50 backdrop-blur-3xl rounded-[44px] p-4 border border-white/5 shadow-2xl transition-all duration-1000">
          <div className="space-y-2 relative">
             <div className="relative h-12 w-full bg-white/5 rounded-[20px] overflow-hidden group border border-white/5">
               ...
               <div className="absolute inset-0 flex items-center justify-between px-6 pointer-events-none">
                 <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">{formatTime(progress)}</span>
                 ...
                 <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">{formatTime(duration)}</span>
               </div>
             </div>
          </div>

          <div className="flex items-center justify-between px-2">
            <button onClick={onToggleShuffle} className={`p-2.5 rounded-full transition-all duration-500 active:scale-75 ${isShuffle ? 'text-accent' : 'text-white/20'}`}>
            ...
            <div className="flex items-center gap-4">
              <button onClick={onPrev} className="text-white/30 p-2 active:scale-[0.85] transition-all hover:text-white"><svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"></path></svg></button>
              <button onClick={onToggle} className="w-14 h-14 flex items-center justify-center bg-white text-black rounded-full active:scale-[0.85] transition-all duration-500 shadow-xl">
              ...
              <button onClick={onNext} className="text-white/30 p-2 active:scale-[0.85] transition-all hover:text-white"><svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"></path></svg></button>
            </div>
            <button onClick={onToggleRepeat} className={`p-2.5 rounded-full transition-all duration-500 active:scale-75 ${repeatMode !== 'off' ? 'text-accent' : 'text-white/20'}`}>
               <div className="relative">
          ...
          <div className="flex bg-white/5 rounded-[28px] p-1 border border-white/5 transition-all duration-500">
            {['player', 'lyrics', 'queue'].map(t => (
              <button 
                key={t} 
                onClick={() => setActiveTab(t as any)} 
                className={`flex-1 py-2.5 rounded-[22px] text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 ${activeTab === t ? 'bg-white text-black shadow-lg scale-100' : 'text-white/20 hover:text-white/40 scale-[0.98]'}`}
              >
                {t}
              </button>
            ))}
