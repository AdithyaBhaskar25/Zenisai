...
const PlayerFull: React.FC<PlayerFullProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black text-white flex flex-col landscape:flex-row overflow-hidden font-sans select-none">
      ...
      {/* --- LEFT PANEL: PLAYER CORE (MOBILE GESTURE / DESKTOP DASHBOARD) --- */}
      <section className="relative flex-1 flex flex-col h-full z-10 landscape:max-w-[340px] lg:landscape:max-w-[420px] landscape:border-r border-white/5 overflow-hidden">
        ...
        <header className="flex items-center justify-between p-6 landscape:pt-10 shrink-0">
          ...
          <div className="landscape:hidden flex gap-3">
             <button onClick={onDownload} className="p-3 bg-white/5 rounded-full"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m4 4V4"></path></svg></button>
             <button onClick={onShowPlaylistModal} className="p-3 bg-white/5 rounded-full"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg></button>
             <button onClick={() => setShowSleepDropdown(!showSleepDropdown)} className={`p-3 rounded-full ${sleepTimer ? 'bg-white text-black' : 'bg-white/5'}`}>
        ...
        {/* Hero Section: Artwork (Tapping art toggles play on all devices) */}
        <div className="flex-1 flex flex-col items-center justify-center px-8 gap-6 landscape:gap-10 min-h-0 landscape:justify-start landscape:pt-4">
          <div 
            onClick={onToggle}
            onTouchStart={handleTouchStart}
            onTouchEnd={handleTouchEnd}
            className="relative w-full max-w-[280px] landscape:max-w-[200px] aspect-square shrink-0 cursor-pointer"
          >
        ...
        <div className="p-10 landscape:p-12 space-y-8 shrink-0">
          {/* Desktop Only Buttons (Visible in Landscape) */}
          <div className="hidden landscape:flex items-center justify-center gap-8 mb-4">
             <button onClick={onPrev} className="p-2 opacity-60 hover:opacity-100 active:scale-90 transition-all">
                <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"></path></svg>
             </button>
             <button onClick={onToggle} className="w-14 h-14 bg-white text-black rounded-full flex items-center justify-center shadow-xl hover:scale-105 active:scale-95 transition-all">
             ...
             <button onClick={onNext} className="p-2 opacity-60 hover:opacity-100 active:scale-90 transition-all">
                <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"></path></svg>
             </button>
          </div>

          <div className="flex items-center justify-between landscape:justify-center landscape:gap-12 px-4">
            <button onClick={onToggleShuffle} className={`p-2 transition-all ${isShuffle ? 'text-white' : 'opacity-20'}`}>
            ...
            <button onClick={() => setShowQueueModal(true)} className="p-4 bg-white/10 rounded-[32px] active:scale-90 transition-all landscape:hidden">
      ...
      {/* --- RIGHT PANEL: DASHBOARD (DESKTOP/LANDSCAPE) --- */}
        ...
        <nav className="flex items-center justify-between px-10 py-10 shrink-0">
          <div className="flex gap-10">
            {(['lyrics', 'queue'] as const).map(t => (
              <button key={t} onClick={() => setActiveTab(t)} className={`text-[11px] font-black uppercase tracking-[0.4em] transition-all ${activeTab === t ? 'text-white border-b-2 border-white pb-2' : 'opacity-20'}`}>{t}</button>
            ))}
          </div>
          
          <div className="flex gap-4 items-center">
             <button onClick={onDownload} className="p-3 bg-white/5 rounded-full hover:bg-white/10 transition-all"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m4 4V4"></path></svg></button>
             <button onClick={onShowPlaylistModal} className="p-3 bg-white/5 rounded-full hover:bg-white/10 transition-all"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg></button>
             <div className="relative">
             ...
             <div className="space-y-3 py-8">
               {queue.map((qs, i) => (
                 <div key={qs.id} draggable onDragStart={() => (dragItemRef.current = i)} onDragOver={(e) => { e.preventDefault(); if (dragItemRef.current !== null && dragItemRef.current !== i) { onMoveQueueItem(dragItemRef.current, i); dragItemRef.current = i; } }} className={`flex items-center gap-4 p-5 rounded-[40px] transition-all border ${qs.id === song.id ? 'bg-white/10 border-white/20' : 'bg-white/5 border-transparent'}`}>
                   ...
                   <button onClick={() => onRemoveFromQueue(qs.id)} className="p-3 opacity-20 hover:opacity-100 hover:text-red-500 transition-all"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                 </div>
               ))}
             </div>
          )}
      ...
      {/* --- GLOBAL HORIZON WAVEBAR (SCRUBBER) --- */}
      <div 
      ...
      {showLyricsModal && (
        <div className="fixed inset-0 z-[300] bg-black animate-in slide-in-from-bottom duration-500 landscape:hidden">
          ...
          <div className="relative h-full flex flex-col p-10 pt-24">
            <button onClick={() => setShowLyricsModal(false)} className="absolute top-10 right-10 p-3 bg-white/10 rounded-full active:scale-90"><svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M6 18L18 6"></path></svg></button>
        ...
        <div className="fixed inset-0 z-[300] bg-[#050505] animate-in slide-in-from-bottom duration-500 landscape:hidden">
           <div className="relative h-full flex flex-col p-8 pt-20">
              <header className="flex justify-between items-center mb-10 px-4">
                ...
                <button onClick={() => setShowQueueModal(false)} className="p-3 bg-white/10 rounded-full"><svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M6 18L18 6"></path></svg></button>
              </header>
              <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pb-32">
                {queue.map((qs, i) => (
                  <div key={qs.id} draggable onDragStart={() => (dragItemRef.current = i)} onDragOver={(e) => { e.preventDefault(); if (dragItemRef.current !== null && dragItemRef.current !== i) { onMoveQueueItem(dragItemRef.current, i); dragItemRef.current = i; } }} className="flex items-center gap-4 p-5 bg-white/5 rounded-[40px] border border-white/5 active:scale-95 transition-all touch-none">
                    ...
                    <button onClick={() => onRemoveFromQueue(qs.id)} className="p-3 opacity-20"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                  </div>
                ))}
              </div>
           </div>
