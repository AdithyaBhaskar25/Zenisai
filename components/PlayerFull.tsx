...
const PlayerFull: React.FC<PlayerFullProps> = ({ 
  ...
  // --- LYRIC ENGINE (Robust Tiered Fetch) ---
  useEffect(() => {
  ...
  // SMOOTH AUTO-SCROLL FIX: Center-aligned and padding aware
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <section className="relative flex-1 flex flex-col h-full z-10 landscape:max-w-[340px] lg:landscape:max-w-[400px] landscape:border-r border-white/5 overflow-hidden">
        ...
        <header className="flex items-center justify-between p-6 landscape:pt-10 shrink-0">
          ...
          <div className="landscape:hidden flex gap-3">
             <button onClick={onDownload} className="p-3 bg-white/5 rounded-full">
               <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m4 4V4"></path></svg>
             </button>
             <button onClick={onShowPlaylistModal} className="p-3 bg-white/5 rounded-full">
             ...
             <div className="relative">
                ...
                {showSleepDropdown && (
                  <div className="absolute right-0 mt-3 w-40 bg-zinc-900/95 border border-white/10 rounded-3xl p-2 shadow-2xl z-50">
                    {[null, 60, 900, 1800].map(v => (
                      <button key={String(v)} onClick={() => {setSleepTimer(v); setShowSleepDropdown(false)}} className="w-full text-left p-4 hover:bg-white/5 rounded-2xl text-[10px] font-black uppercase tracking-widest">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8 gap-6 landscape:gap-10 min-h-0 landscape:justify-start landscape:pt-4">
          ...
          >
            <img src={song.artwork} className={`w-full h-full object-cover rounded-[56px] shadow-2xl border border-white/5 transition-all duration-700 ${isPlaying ? 'scale-100' : 'scale-90 opacity-40 blur-[4px]'}`} />
            ...
            >
              <svg className="w-6 h-6" fill={isFavorite ? "currentColor" : "none"} stroke="white" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
            </button>
          </div>
        ...
        {/* Global Action Row: Shuffle and Repeat (Only these in Landscape Left) */}
        <div className="p-10 landscape:p-12 space-y-8 shrink-0">
          <div className="flex items-center justify-between landscape:justify-center landscape:gap-16 px-4">
            <button onClick={onToggleShuffle} className={`p-2 transition-all ${isShuffle ? 'text-white' : 'opacity-20'}`}>
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>
            </button>
            ...
            <button onClick={onToggleRepeat} className={`p-2 transition-all ${repeatMode !== 'off' ? 'text-white' : 'opacity-20'}`}>
              <div className="relative">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
             ...
             <div className="space-y-3 py-8">
               {queue.map((qs, i) => (
                 <div key={qs.id} draggable onDragStart={() => (dragItemRef.current = i)} onDragOver={(e) => { e.preventDefault(); if (dragItemRef.current !== null && dragItemRef.current !== i) { onMoveQueueItem(dragItemRef.current, i); dragItemRef.current = i; } }} className={`flex items-center gap-4 p-5 rounded-[40px] transition-all border ${qs.id === song.id ? 'bg-white/10 border-white/20' : 'bg-white/5 border-transparent'}`}>
                   ...
                   <button onClick={() => onRemoveFromQueue(qs.id)} className="p-3 opacity-20 hover:opacity-100 hover:text-red-500 transition-all">
      ...
      >
        ...
        {/* Rounded Scrubber Knob */}
        <div 
            className="absolute top-0 bottom-0 w-2.5 h-full bg-white shadow-[0_0_20px_white] rounded-full pointer-events-none transition-all duration-300"
      ...
      {showLyricsModal && (
        <div className="fixed inset-0 z-[300] bg-black animate-in slide-in-from-bottom duration-500 landscape:hidden">
          ...
          <div className="relative h-full flex flex-col p-10 pt-24">
            <button onClick={() => setShowLyricsModal(false)} className="absolute top-10 right-10 p-3 bg-white/10 rounded-full active:scale-90">
               <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M6 18L18 6"></path></svg>
            </button>
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pb-32 px-2">
                {queue.map((qs, i) => (
                  ...
                  >
                    ...
                    <button onClick={() => onRemoveFromQueue(qs.id)} className="p-3 opacity-20">
                       <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                  </div>
                ))}
