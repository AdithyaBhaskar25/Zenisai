    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full px-6 pt-10 pb-4">
        <header className="flex justify-between items-center h-10 mb-4">
          ...
          <div className="text-center flex-1 px-4"><p className="text-[10px] font-bold text-white/70 truncate">{song.album || 'Zenisai'}</p></div>
          <div className="relative">
            <button onClick={() => { setShowDropdown(!showDropdown); setShowSleepTimerMenu(false); }} className="p-2 bg-white/5 rounded-full"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 5v.01M12 12v.01M12 19v.01" /></svg></button>
            {showDropdown && (
              <div className="absolute right-0 top-12 w-48 bg-zinc-900 border border-white/10 rounded-2xl p-2 shadow-2xl z-50">
                {!showSleepTimerMenu ? (
                  <div className="space-y-1">
                    <button onClick={() => { onDownload(); setShowDropdown(false); }} className="w-full flex items-center gap-3 p-3 text-[10px] font-bold uppercase text-white/70 hover:bg-white/5 rounded-xl"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1" /></svg> Save</button>
                    <button onClick={() => { onShowPlaylistModal(); setShowDropdown(false); }} className="w-full flex items-center gap-3 p-3 text-[10px] font-bold uppercase text-white/70 hover:bg-white/5 rounded-xl"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg> Add</button>
                  ...
                  <div className="space-y-1">
                    ...
                    {[null, 60, 900, 1800].map((v) => (
                      <button key={String(v)} onClick={() => { setSleepTimer(v); setShowDropdown(false); }} className={`w-full text-left p-3 text-[10px] font-bold uppercase rounded-xl ${sleepTimer === v ? 'text-accent' : 'text-white/70 hover:bg-white/5'}`}>
                        {v === null ? 'Off' : v === 60 ? '60 Sec' : `${v/60} Mins`}
                      </button>
                    ))}
                  </div>
                )}
        ...
        <footer className="mt-4 space-y-4 bg-zinc-900/40 backdrop-blur-2xl rounded-[36px] p-5 border border-white/5 shadow-2xl">
          <div className="relative h-9 w-full bg-white/5 rounded-[18px] overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-center justify-between px-5 pointer-events-none text-[8px] font-black text-white/20 uppercase tracking-[0.2em]"><span>{formatTime(progress)}</span><span>{formatTime(duration)}</span></div>
          </div>

          <div className="flex items-center justify-between px-1">
            <button onClick={onToggleShuffle} className={`p-2 transition-all ${isShuffle ? 'text-accent' : 'text-white/20'}`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg></button>
            ...
            <button onClick={onToggleRepeat} className={`p-2 transition-all ${repeatMode !== 'off' ? 'text-accent' : 'text-white/20'}`}>
              <div className="relative">
                ...
                {repeatMode === 'one' && <span className="absolute -top-1 -right-1 text-[7px] font-bold bg-accent text-white rounded-full w-3 h-3 flex items-center justify-center">1</span>}
              </div>
            </button>
          </div>
